    ...
    <div className={`atlas-shell atlas-world-shell theme-${theme} ${displayMode === "globe" ? "is-globe-mode" : ""} ${displayMode === "tactical" ? "is-tactical-mode" : ""} ${analysisMode === "evolution" ? "is-evolution-mode" : ""}`}>
      ...
      <main id="observatoire" className="world-observatory">
        <section ref={mapStageRef} className={`world-map-stage ${areFiltersVisible ? "" : "filters-hidden"}`} aria-label="Carte mondiale des relations géopolitiques">
          ...
          <section className={`relation-timeline ${isTimelineExpanded ? "is-expanded" : ""}`} aria-label="Timeline des relations"><button type="button" className="timeline-title" onClick={() => setIsTimelineExpanded((value) => !value)} aria-expanded={isTimelineExpanded}><Clock3 size={15} aria-hidden="true" /><span>TIMELINE</span><ChevronRight size={14} aria-hidden="true" /></button><div className="timeline-boundary-dates"><span><small>DE</small>{formatDateFr(periodStartDate)}</span><span><small>À</small>{formatDateFr(periodEndDate)}</span></div><div className="timeline-range-control"><span className="timeline-handle-label is-start" style={{ left: `${((periodStartDate.getFullYear() - 1858) / (2025 - 1858)) * 100}%` }}>{formatDateFr(periodStartDate)}</span><span className="timeline-handle-label is-end" style={{ left: `${((periodEndDate.getFullYear() - 1858) / (2025 - 1858)) * 100}%` }}>{formatDateFr(periodEndDate)}</span><input type="range" className="timeline-range-start" min="1858" value={periodStartDate.getFullYear()} max={periodEndDate.getFullYear()} onChange={(event) => { const date = new Date(periodStartDate); date.setFullYear(Number(event.target.value)); setPeriodStartDate(date); setTimelineYear(date.getFullYear()); }} aria-label="Date de début de la timeline" /><input type="range" className="timeline-range-end" max="2025" value={periodEndDate.getFullYear()} min={periodStartDate.getFullYear()} onChange={(event) => { const date = new Date(periodEndDate); date.setFullYear(Number(event.target.value)); setPeriodEndDate(date); setTimelineYear(date.getFullYear()); }} aria-label="Date de fin de la timeline" /></div>{isTimelineExpanded && <div className="timeline-date-editor"><div className="date-range-inputs"><label>DE<input type="date" value={periodStartDate.toISOString().slice(0, 10)} max={periodEndDate.toISOString().slice(0, 10)} onChange={(event) => { const date = new Date(`${event.target.value}T00:00:00`); setPeriodStartDate(date); setTimelineYear(date.getFullYear()); }} /></label><label>À<input type="date" value={periodEndDate.toISOString().slice(0, 10)} min={periodStartDate.toISOString().slice(0, 10)} onChange={(event) => { const date = new Date(`${event.target.value}T00:00:00`); setPeriodEndDate(date); setTimelineYear(date.getFullYear()); }} /></label></div><Popover><PopoverTrigger asChild><button className="calendar-range-trigger" type="button"><CalendarDays size={14} /> Choisir au calendrier</button></PopoverTrigger><PopoverContent className="atlas-date-popover" align="center"><Calendar mode="range" selected={{ from: periodStartDate, to: periodEndDate }} onSelect={(range) => { if (range?.from) { setPeriodStartDate(range.from); setTimelineYear(range.from.getFullYear()); } if (range?.to) { setPeriodEndDate(range.to); setTimelineYear(range.to.getFullYear()); } }} captionLayout="dropdown" fromYear={1858} toYear={2025} numberOfMonths={1} /></PopoverContent></Popover></div>}<p>{activeRelations.length} liens actifs</p></section>
