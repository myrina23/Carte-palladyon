      ...
      <main id="observatoire" className="world-observatory">
        <section ref={mapStageRef} className={`world-map-stage ${areFiltersVisible ? "" : "filters-hidden"}`} aria-label="Carte mondiale des relations géopolitiques">
          ...
          <section className="world-filter-panel world-filter-panel-merged relation-filter-panel intro-animate delay-1" aria-label="Filtres principaux">
            ...
            <div className="filter-map-controls" aria-label="Projection et visibilité"><div className="display-mode-switch"><button type="button" title="Carte plane" aria-pressed={displayMode === "map"} className={displayMode === "map" ? "is-active" : ""} onClick={() => toggleDisplayMode("map")}>2D</button><button type="button" title="Globe" aria-pressed={displayMode === "globe"} className={displayMode === "globe" ? "is-active" : ""} onClick={() => toggleDisplayMode("globe")}>GLOBE</button><button type="button" title="Vue tactique" aria-pressed={displayMode === "tactical"} className={displayMode === "tactical" ? "is-active" : ""} onClick={() => toggleDisplayMode("tactical")}>TAC</button></div><button className="filter-visibility-toggle" type="button" onClick={() => setAreFiltersVisible((visible) => !visible)} aria-pressed={areFiltersVisible} title={areFiltersVisible ? "Masquer les filtres" : "Afficher les filtres"}>{areFiltersVisible ? <Eye size={15} /> : <EyeOff size={15} />}</button></div>
            ...
            <div className="filter-group"><p>RÉGIONS · CHOIX MULTIPLES</p><div className="filter-pills region-pills">{REGION_FILTERS.map((region) => <button key={region.id} type="button" className={activeRegions.includes(region.id) ? "is-selected" : ""} onClick={() => toggleRegionFilter(region.id)} aria-pressed={activeRegions.includes(region.id)}>{region.label}</button>)}</div></div>
