...
import { AtlasDock, AtlasDockIcon, AtlasDockItem } from "@/components/palladyon/AtlasDock";
import { AtlasMorphingContent, AtlasMorphingPopover, AtlasMorphingTrigger } from "@/components/palladyon/AtlasMorphingPopover";
  ...
  return (
    <div className={`atlas-shell atlas-world-shell theme-${theme} ${displayMode === "globe" ? "is-globe-mode" : ""} ${displayMode === "tactical" ? "is-tactical-mode" : ""} ${analysisMode === "evolution" ? "is-evolution-mode" : ""}`}>
      <header className="atlas-header" aria-label="Navigation principale">
        <a className="atlas-brand" href="#observatoire" aria-label="Atlas Flux — observatoire mondial"><svg className="atlas-mark atlas-logo-vector" viewBox="0 0 64 64" aria-hidden="true"><path d="M5 32 27 8l8 9-13 15 13 15-8 9Z" /><path d="M59 32 37 8l-8 9 13 15-13 15 8 9Z" /><path d="M32 22 42 32 32 42 22 32Z" /></svg><span>ATLAS <em>FLUX</em></span></a>
      ...
      <main id="observatoire" className="world-observatory">
        <section ref={mapStageRef} className={`world-map-stage ${areFiltersVisible ? "" : "filters-hidden"}`} aria-label="Carte mondiale des relations géopolitiques">
          ...
          >
            {displayMode !== "globe" && <MapLibre key={`map-${viewKey}-${displayMode}-${theme}`} ref={mapRef} initialViewState={{ ...projectionView, pitch: displayMode === "tactical" ? 58 : 0, bearing: displayMode === "tactical" ? -14 : 0 } as any} mapStyle={(theme === "light" ? ATLAS_LIGHT_VECTOR_STYLE : ATLAS_DARK_VECTOR_STYLE) as any} attributionControl={false} canvasContextAttributes={{ preserveDrawingBuffer: true }} style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}><NavigationControl position="bottom-right" showCompass={displayMode === "tactical"} /></MapLibre>}
          </DeckGL>
          ...
          <div className={`world-intro intro-animate ${isIntroVisible ? "" : "is-dismissed"}`}><p className="eyebrow"><Radar size={14} aria-hidden="true" /> {displayMode === "globe" ? "GLOBE DES INTERDÉPENDANCES" : displayMode === "tactical" ? "VUE TACTIQUE LOCALE" : "OBSERVATOIRE GÉOPOLITIQUE"}</p><h1>Relier les<br /><i>forces</i> en présence.</h1><p>{displayMode === "tactical" ? "Approchez une zone, un pays ou une organisation pour examiner sa densité relationnelle et les événements de conflit sourcés." : "Un clic révèle le réseau d’un acteur ; un second clic sur un autre acteur lance leur comparaison directe. Filtrez ensuite les liens par nature et par période."}</p></div>
          <button className="map-filter-visibility-toggle" type="button" onClick={() => setAreFiltersVisible((visible) => !visible)} aria-pressed={areFiltersVisible} title={areFiltersVisible ? "Masquer les filtres" : "Afficher les filtres"}>{areFiltersVisible ? <Eye size={16} /> : <EyeOff size={16} />}</button>
          ...
          <section className="world-filter-panel world-filter-panel-merged relation-filter-panel intro-animate delay-1" aria-label="Filtres principaux">
            ...
            <div className="filter-group filter-typologies"><p><Layers3 size={13} aria-hidden="true" /> LIENS · CHOIX MULTIPLES</p><div className="relation-type-grid relation-type-grid-expanded">{RELATION_TYPES.map((type) => <label key={type.id} data-relation={type.short} className={visibleRelationTypes[type.id] ? "is-active" : ""}><i style={{ backgroundColor: `rgb(${type.color.join(" ")})` }} /><span>{type.label}</span><Switch checked={visibleRelationTypes[type.id]} onCheckedChange={() => selectRelationType(type.id)} aria-label={`Afficher les liens ${type.label}`} /></label>)}</div></div>
          ...
          <section className="relation-timeline" aria-label="Timeline des relations"><Popover><PopoverTrigger asChild><button type="button" className="timeline-title" aria-label="Modifier la période de lecture"><Clock3 size={15} aria-hidden="true" /><span>TIMELINE</span><CalendarDays size={14} aria-hidden="true" /></button></PopoverTrigger><PopoverContent className="atlas-date-popover timeline-date-popover" align="center" side="top"><div className="timeline-date-editor"><p><CalendarDays size={14} /> PÉRIODE DE LECTURE</p><Calendar mode="range" selected={{ from: periodStartDate, to: periodEndDate }} onSelect={(range) => { if (range?.from) { setPeriodStartDate(range.from); setTimelineYear(range.from.getFullYear()); } if (range?.to) { setPeriodEndDate(range.to); setTimelineYear(range.to.getFullYear()); } }} captionLayout="dropdown" fromYear={1858} toYear={2025} numberOfMonths={1} /><div className={`timeline-wheel-picker ${isPeriodAnimating ? "is-changing" : ""}`} aria-label="Sélecteur de période à listes défilantes">{(["start", "end"] as const).map((boundary) => { const date = boundary === "start" ? periodStartDate : periodEndDate; return <div key={boundary}><b>{boundary === "start" ? "DE" : "À"} {formatDateFr(date)}</b><span><select aria-label={`${boundary === "start" ? "Jour de début" : "Jour de fin"}`} value={date.getDate()} onChange={(event) => updatePeriodDate(boundary, "day", Number(event.target.value))}>{Array.from({ length: 31 }, (_, index) => <option key={index + 1} value={index + 1}>{String(index + 1).padStart(2, "0")}</option>)}</select><select aria-label={`${boundary === "start" ? "Mois de début" : "Mois de fin"}`} value={date.getMonth()} onChange={(event) => updatePeriodDate(boundary, "month", Number(event.target.value))}>{Array.from({ length: 12 }, (_, index) => <option key={index} value={index}>{String(index + 1).padStart(2, "0")}</option>)}</select><select aria-label={`${boundary === "start" ? "Année de début" : "Année de fin"}`} value={date.getFullYear()} onChange={(event) => updatePeriodDate(boundary, "year", Number(event.target.value))}>{Array.from({ length: 168 }, (_, index) => 1858 + index).map((year) => <option key={year} value={year}>{year}</option>)}</select></span></div>; })}</div></div></PopoverContent></Popover><div className="timeline-range-control" style={{ "--timeline-start": `${((periodStartDate.getFullYear() - 1858) / (2025 - 1858)) * 100}%`, "--timeline-end": `${((periodEndDate.getFullYear() - 1858) / (2025 - 1858)) * 100}%` } as React.CSSProperties}><span className="timeline-handle-label is-start" style={{ left: "var(--timeline-start)" }}>{formatDateFr(periodStartDate)}</span><span className="timeline-handle-label is-end" style={{ left: "var(--timeline-end)" }}>{formatDateFr(periodEndDate)}</span><Slider className="atlas-palladyon-range" min={1858} max={2025} step={1} value={[periodStartDate.getFullYear(), periodEndDate.getFullYear()]} minStepsBetweenThumbs={1} onValueChange={([startYear, endYear]) => { const start = new Date(periodStartDate); const end = new Date(periodEndDate); start.setFullYear(startYear); end.setFullYear(endYear); setPeriodStartDate(start); setPeriodEndDate(end); setTimelineYear(endYear); }} aria-label="Plage temporelle Atlas Flux" /></div><p>{activeRelations.length} liens actifs</p></section>

          <aside className={`world-detail-panel ${selectedCountry || selectedOrganization || selectedRelation ? "is-open" : ""} ${selectedRelation ? "relation-open" : ""}`} aria-live="polite" aria-label="Fiche acteur ou relation">
            ...
            {(selectedCountry || selectedOrganization || selectedRelation) && (() => { const actorId = selectedRelation ? undefined : selectedCountry?.iso3 ?? selectedOrganization?.id; const relationSources = selectedRelation ? activeRelations.filter((relation) => relation.id === selectedRelation.id || relation.source.id === selectedRelation.source.id || relation.target.id === selectedRelation.target.id).map(relationReference) : activeRelations.filter((relation) => relation.source.id === actorId || relation.target.id === actorId).map(relationReference); const actorSources = selectedCountry ? [{ label: `Wikipédia — ${selectedCountry.name}`, url: countryWikipediaUrl(selectedCountry.name) }, ...(selectedCountry.entityKind !== "territory" ? [{ label: `Banque mondiale — ${selectedCountry.name}`, url: `https://data.worldbank.org/country/${selectedCountry.iso2.toLowerCase()}` }] : [])] : selectedOrganization ? [{ label: "Wikipédia", url: ORGANIZATION_REFERENCES[selectedOrganization.id].wikipedia }, { label: "Site officiel", url: ORGANIZATION_REFERENCES[selectedOrganization.id].official }] : selectedRelation ? [relationReference(selectedRelation)] : []; const sources = Array.from(new Map([...actorSources, ...relationSources].map((source) => [source.url, source])).values()); return <section className="detail-source-register" aria-label="Sources de la fiche"><header><span><BookOpen size={14} /> SOURCES DE LA FICHE</span><b aria-label={`${sources.length} sources au total`}>{sources.length}</b></header><ScrollArea className="detail-source-scroll"><div>{sources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer"><span>{source.label}</span><ExternalLink size={12} /></a>)}</div></ScrollArea></section>; })()}
            ...
            {selectedRelation && (() => { const reference = relationReference(selectedRelation); const type = RELATION_TYPES.find((entry) => entry.id === selectedRelation.type) ?? RELATION_TYPES[0]; const relatedSources = activeRelations.filter((relation) => relation.source.id === selectedRelation.source.id || relation.target.id === selectedRelation.target.id).slice(0, 12); return <AtlasMorphingPopover className="relation-in-detail" open={Boolean(selectedRelation)} onOpenChange={(open) => { if (!open) setSelectedRelation(null); }}><AtlasMorphingTrigger className="relation-morphing-trigger">Ouvrir la fiche relationnelle</AtlasMorphingTrigger><AtlasMorphingContent className="relation-detail-inline"><div className="relation-profile-head"><p className="eyebrow"><i style={{ backgroundColor: `rgb(${type.color.join(" ")})` }} /> {type.label.toUpperCase()}</p><h3>{selectedRelation.source.name}<span>→</span>{selectedRelation.target.name}</h3></div><div className="relation-quick-actions"><button type="button" onClick={() => setSelectedActorId(selectedRelation.source.id)}><UsersRound size={14} /> Voir les acteurs</button><a href={reference.url} target="_blank" rel="noreferrer"><ExternalLink size={14} /> Voir la relation</a><button type="button" onClick={() => navigator.clipboard?.writeText(window.location.href)}><ArrowUpRight size={14} /> Partager</button></div><section className="relation-summary"><p>RÉSUMÉ</p><h4>{selectedRelation.title}</h4><span>{selectedRelation.detail}</span></section><section className="relation-history-card"><p><Clock3 size={14} /> HISTORIQUE</p><div><strong>{selectedRelation.start ?? "—"}</strong><span>{selectedRelation.end ? `→ ${selectedRelation.end}` : "→ aujourd’hui"}</span></div></section><section className="relation-source-scroll-section"><p><BookOpen size={14} /> SOURCES ASSOCIÉES</p><ScrollArea className="palladyon-source-scroll"><div className="relation-source-scroll-list"><a className="relation-source-link" href={reference.url} target="_blank" rel="noreferrer"><BookOpen size={14} /> {reference.label} <ExternalLink size={14} /></a>{relatedSources.map((relation) => { const source = relationReference(relation); return <a key={relation.id} href={source.url} target="_blank" rel="noreferrer"><i style={{ backgroundColor: `rgb(${relationColor(relation.type).join(" ")})` }} />{source.label} · {relation.title}<ExternalLink size={12} /></a>; })}</div></ScrollArea></section><div className="relation-bottom-actions"><button type="button" onClick={downloadScenarioSnapshot}><Printer size={15} /> PDF</button><button type="button" onClick={openContribution}><PenLine size={15} /> Correction</button></div></AtlasMorphingContent></AtlasMorphingPopover>; })()}
          </aside>
          ...
          {selectedRelation && (() => { const reference = relationReference(selectedRelation); const type = RELATION_TYPES.find((entry) => entry.id === selectedRelation.type) ?? RELATION_TYPES[0]; const intensity = Math.min(10, Math.max(2, Math.round((selectedRelation.detail.length + (selectedRelation.end ? 30 : 10)) / 24))); const relatedSources = activeRelations.filter((relation) => relation.source.id === selectedRelation.source.id || relation.target.id === selectedRelation.target.id).slice(0, 12); return <AtlasMorphingPopover open={Boolean(selectedRelation)} onOpenChange={(open) => { if (!open) setSelectedRelation(null); }}><AtlasMorphingTrigger className="relation-morphing-trigger">Ouvrir la fiche relationnelle</AtlasMorphingTrigger><AtlasMorphingContent className="world-relation-panel palladyon-relation-panel"><button className="detail-close" type="button" onClick={() => setSelectedRelation(null)} aria-label="Fermer le détail"><X size={17} /></button><div className="relation-profile-head"><p className="eyebrow"><i style={{ backgroundColor: `rgb(${type.color.join(" ")})` }} /> {type.label.toUpperCase()}</p><h3>{selectedRelation.source.name}<span>→</span>{selectedRelation.target.name}</h3></div><div className="relation-quick-actions"><button type="button" onClick={() => setSelectedActorId(selectedRelation.source.id)}><UsersRound size={14} /> Voir les acteurs</button><a href={reference.url} target="_blank" rel="noreferrer"><ExternalLink size={14} /> Voir la relation</a><button type="button" onClick={() => navigator.clipboard?.writeText(window.location.href)}><ArrowUpRight size={14} /> Partager</button></div><div className="relation-facts"><div><span>PÉRIODE</span><strong>{relationPeriodLabel(selectedRelation)}</strong></div><div><span>ÉCHELLE</span><strong>{activeRegions.includes("all") ? "Mondiale" : activeRegions.length > 1 ? "Multi-régionale" : "Régionale"}</strong></div><div><span>INTENSITÉ</span><strong><i><b style={{ width: `${intensity * 10}%`, backgroundColor: `rgb(${type.color.join(" ")})` }} /></i>{intensity}/10</strong></div></div><section className="relation-summary"><p>RÉSUMÉ</p><h4>{selectedRelation.title}</h4><span>{selectedRelation.detail}</span></section><section className="relation-history-card"><p><Clock3 size={14} /> HISTORIQUE <em>{selectedRelation.start ? "1 événement" : "Structurel"}</em></p><div><strong>{selectedRelation.start ?? "—"}</strong><span>{selectedRelation.end ? `→ ${selectedRelation.end}` : "→ aujourd’hui"}</span></div></section><section className="relation-source-scroll-section"><p><BookOpen size={14} /> SOURCES ASSOCIÉES</p><ScrollArea className="palladyon-source-scroll"><div className="relation-source-scroll-list"><a className="relation-source-link" href={reference.url} target="_blank" rel="noreferrer"><BookOpen size={14} /> {reference.label} <ExternalLink size={14} /></a>{relatedSources.map((relation) => { const source = relationReference(relation); return <a key={relation.id} href={source.url} target="_blank" rel="noreferrer"><i style={{ backgroundColor: `rgb(${relationColor(relation.type).join(" ")})` }} />{source.label} · {relation.title}<ExternalLink size={12} /></a>; })}</div></ScrollArea></section><div className="relation-bottom-actions"><button type="button" onClick={downloadScenarioSnapshot}><Printer size={15} /> PDF</button><button type="button" onClick={openContribution}><PenLine size={15} /> Correction</button></div></AtlasMorphingContent></AtlasMorphingPopover>; })()}
          </section>
      </main>

      <input id="timeline-fullscreen-input" className="timeline-fullscreen-input" type="checkbox" aria-label="Afficher la Timeline en plein écran" />
      <label htmlFor="timeline-fullscreen-input" className="timeline-fullscreen-control"><span aria-hidden="true">⤢</span><span>Timeline plein écran</span></label>

      <section className="palladyon-global-dock" aria-label="Actions cartographiques globales"><AtlasDock><AtlasDockItem label="Comparer" onClick={() => setIsComparatorOpen(true)}><AtlasDockIcon><GitCompareArrows size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label="Recentrer" onClick={resetView}><AtlasDockIcon><LocateFixed size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label="Annoter" onClick={openContribution}><AtlasDockIcon><PenLine size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label={`Enregistrer${savedSnapshots.length ? ` (${savedSnapshots.length})` : ""}`} onClick={saveNamedSnapshot}><AtlasDockIcon><BookOpen size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label="Exporter le relevé PDF" onClick={downloadScenarioSnapshot}><AtlasDockIcon><Printer size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label="Rapport PDF" onClick={downloadScenarioSnapshot}><AtlasDockIcon><FileText size={18} /></AtlasDockIcon></AtlasDockItem><AtlasDockItem label="Partager" onClick={() => void shareSavedSnapshot()}><AtlasDockIcon><Share2 size={18} /></AtlasDockIcon></AtlasDockItem></AtlasDock>{shareNotice && <span className="share-notice" role="status">{shareNotice}</span>}</section>
